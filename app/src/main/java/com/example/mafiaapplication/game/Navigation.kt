...
@Composable
fun Navigation(game: MafiaGame, bluetoothHelper: BluetoothHelper, sharedPreferencesHelper: SharedPreferencesHelper) {
    ...
    NavHost(navController = navController, startDestination = Screen.StartGameScreen.route) {
        ...
        composable(route = Screen.RolePickerScreen.route, arguments = listOf()) {
           RolePickerStage(navController = navController, game = game, mainActivity = MainActivity(), sharedPreferencesHelper = sharedPreferencesHelper)
        }
        composable(route = Screen.NightStageScreen.route, arguments = listOf()) {
            NightStage(navController = navController, game = game, sharedPreferencesHelper = sharedPreferencesHelper)
        }
        composable(route = Screen.DayStageScreen.route, arguments = listOf()) {
            DayStage(navController = navController, game = game, sharedPreferencesHelper = sharedPreferencesHelper)
        }
        composable(route = Screen.VoteMainStageScreen.route, arguments = listOf()) {
            VoteMainStage(navController = navController, game = game, sharedPreferencesHelper = sharedPreferencesHelper)
        }
        composable(route = Screen.VoteStageScreen.route, arguments = listOf()) {
            VoteStage(navController = navController, game = game, sharedPreferencesHelper = sharedPreferencesHelper)
        }
        composable(route = Screen.LastWordsScreen.route, arguments = listOf()) {
            LastWordsStage(navController = navController, game = game, sharedPreferencesHelper = sharedPreferencesHelper)
        }
